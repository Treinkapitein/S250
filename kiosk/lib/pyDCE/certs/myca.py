...
"""
Author Richrd Cai
Description: Helper Script for CA Manage
"""
import os
import sys
import getopt
...
if home is None:
	...
	if home is None:
	    ...
	    sys.exit(1)
	home = os.path.join(home, ".myca")
home = os.path.normpath(home)
os.putenv("CA_HOME", home)
...
if sys.argv[script] == "init":
    ...
    try:
	os.makedirs(cadb)
        os.makedirs(os.path.join(cadb,"certs"))
	os.makedirs(os.path.join(cadb,"private"))
    except OSError:
	pass
	...
	'L':['localityName', "Locality", "Shanghai", 'match'], \
	'O':['organizationName', "Organization name", "SWSCA-" + socket.gethostname(), 'match'], \
	'OU':['organizationUnitName', "Organization unit name", "", 'optional'], \
	'CN':['commonName', "Common name", "SWS CA", 'supplied'] \
    }
...
# **********************************************************************\n\
\n\
# Configuration file for the CA. This file is generated by myca init.\n\
# DO NOT EDIT!\n\
\n\
###############################################################################\n\
...
\n\
[ myca ]\n\
default_days     = 356    # How long certs are valid.\n\
default_md       = sha1     # The Message Digest type.\n\
preserve         = no      # Keep passed DN ordering?\n\
\n\
[ req ]\n\
default_bits        = 2048\n\
default_keyfile     = $ENV::CA_HOME/ca/db/private/ca_key.pem\n\
default_md          = sha1\n\
prompt              = no\n\
...
# **********************************************************************\n\
\n\
# Configuration file to sign a certificate. This file is generated by myca init.\n\
# DO NOT EDIT!!\n\
\n\
[ ca ]\n\
default_ca = myca\n\
\n\
[ myca ]\n\
dir              = $ENV::CA_HOME/ca/db  # Where everything is kept.\n\
private_key      = $dir/private/ca_key.pem   # The CA Private Key.\n\
certificate      = $dir/ca_cert.pem  # The CA Certificate.\n\
database         = $dir/index.txt           # Database index file.\n\
new_certs_dir    = $dir/certs                     # Default loc for new certs.\n\
...
# certificate. This file is generated by myca init.\n\
# DO NOT EDIT!\n\
\n\
[ req ]\n\
default_bits        = 1024\n\
...
\n\
# The dn section is added by the myca script.\n\
\n\
[dn]\n\
"\
    }
...
#    cnf = open(os.path.join(caroot, "ca.cnf"), "w")
#    cnf.write(config["ca.cnf"])
#    for k,v in DNelements.iteritems():
#	if len(v[2]) > 0:
#            cnf.write(v[0] + "=" + v[2] + "\n")
#    cnf.close()
    ...
    print "ok"
    print "now generating self-signed certificate for Out CA"

    cmd = "openssl req -config " + cacnfname + " -x509 -days 356 -newkey rsa:2048 -out " + \
...
if sys.argv[script] == "request":
    def usage():
	print "usage: " + sys.argv[script] + " [--overwrite] [--server|--user] [--no-password]"
	sys.exit(1)
	...
	opts, args = getopt.getopt(sys.argv[script+1:], "", \
				   [ "overwrite", "server", "user", "no-password" ])
    except getopt.GetoptError:
	usage()
	...
	if  o == "--server":
	    ...
	    commonName = "My Server " + commonName
	elif o == "--user":
	    if type != None:
		usage()
...
if sys.argv[script] == "sign":
    ...
    cmd = "openssl ca -config " + tempname + " -in " + infile
    if(outfile!=None):
	cmd+= " -out " + outfile
    if verbose: print cmd
    status = os.system(cmd)
