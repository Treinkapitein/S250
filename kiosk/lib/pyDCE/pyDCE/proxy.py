...
class RemoteMethod:
    def __init__(self,proxy,obj,method):
            #one_way,compress,ctx,timeout=config.DCE_DEFAULT_TIMEOUT,async=0 ):
        self._obj=obj
        self._proxy = proxy
        self._method = method
        """
    ...
    def __call__(self, *args, **kwargs):
      ...
      #  return self.__send(self.__name, args, kwargs)
        return self._proxy.dce_invoke(self._obj,self._method,args,kwargs)
    #self._one_way,self._compress,self._ctx,to,self._async,args,kwargs)
...
#(request_uuid,reply status,body)
    def _return_reply(self,reply):
        ...
        if status==pyDCE.protocol.SUCCESS:
            DCE_APP().DEBUG("_return_reply success")
            return body
        elif status==pyDCE.protocol.DCE_EXCEPTION:
            DCE_APP().DEBUG("_return_reply except")
            raise body
        
    def dce_invoke(self,obj,method,vargs,kargs):
       #print "&*&*&&*&*&*&"*12, method,vargs, kargs 
       dce_one_way=0
       dce_compress=0
       dce_timeout=config.DCE_DEFAULT_TIMEOUT
       dce_async=0
  
       if kargs.has_key('dce_timeout'):
           dce_timeout=kargs.pop('dce_timeout')
       if kargs.has_key('dce_compress'):
           dce_compress=kargs.pop('dce_compress')
       if kargs.has_key('dce_one_way'):
           dce_one_way=kargs.pop('dce_one_way')
       if kargs.has_key('dce_async'):
           dce_one_way=kargs.pop('dce_async')

       req=pyDCE.protocol.Request(obj,method,(vargs,kargs)) #,0,dce_one_way,dce_compress)
    ...
    #   finally:
       ...
       if self._isReplyGot(rid):
           ...
           return ret
       if dce_timeout==0:
           DCE_APP().DEBUG("[Proxy] waiting with default timeout")
           ...
           while time.time() - tick < config.DCE_DEFAULT_TIMEOUT:
               ...
               if not self._conn._sock:
                   cond.release()
                   DCE_APP().DEBUG("[Proxy] socket error when waiting for reply")
                   raise pyDCE.exception.TimeoutException("Call %s.%s timeout" % (obj,method))
       else:
           DCE_APP().DEBUG("[Proxy] waiting with timeout "+ str(dce_timeout) )
           import time
           tick=time.time()
           while time.time() - tick < dce_timeout:
               #print "^&^%%%%%%%%%%%%%%%%%%&&&&"*10, rid, self._isReplyGot(rid)
               cond.wait(2)
               if self._isReplyGot(rid):
                   break
               if not self._conn._sock:
    ...
    #       DCE_APP().DEBUG("[Proxy] got result")
           ...
           cond.release()
           raise pyDCE.exception.TimeoutException("[Proxy] Call %s.%s timeout" % (obj,method))
    ...
    def __getattr__(self,dce_method_name):
            #dce_one_way=0,dce_compress=0,dce_ctx={},dce_timeout=0,dce_async=0):
        return RemoteMethod(self,self._obj,dce_method_name)
    ...
    def __del__(self):
        DCE_APP().DEBUG("[Proxy] __del__...")
        try:
          #  DCE_APP().DEBUG("[Proxy] connection fd to delete: " + str(self._conn.fileno()))
            del self._conn
           # self._conn.close()
           # self._conn._sock=-1
          #  self._conn=None
...
#        print "getattr..." , name
        if name not in ("__getinitargs__", "__hash__","__eq__","__ne__"):
            ...
            if result==1: # method
                ...
                return RemoteMethod(self,self._obj,name)
    ...
    def dce_invoke(self,obj,method,vargs,kargs):
       ...
       if kargs.has_key('dce_async'):
           dce_one_way=kargs.pop('dce_async')
       req=pyDCE.protocol.Request(obj,method,(vargs,kargs) )  # ,{},dce_one_way,dce_compress)
       p=pyDCE.protocol.Protocol()
       (rid,r)=p.RequestToRaw(req)
       ...
       try:
           ...
           for i in xrange(0,1):
               ...
               DCE_APP().DEBUG("connection fd: "+ str(self._conn.fileno()))
             #  self._conn.waitr(timeout=dce_timeout)
               (reply_rid, reply_body)= self.on_recv(timeout=dce_timeout)
               if reply_rid==rid:
                   return self._return_reply(reply_body)
           if reply_rid != rid:
               raise pyDCE.exception.ProtocolException("Reply RID not matched, wanted:%s, got:%s" %(rid,reply_rid) )
       except Exception,ex:
        #   self._conn.close()
        #   self._conn=None
           raise ex
...
#        self.dce_setattr(name,value)
    ...
    def on_recv(self,timeout=config.DCE_DEFAULT_TIMEOUT):
        DCE_APP().DEBUG("[SimpleProxy] on_rec...")
        p=pyDCE.protocol.Protocol()
        head_size=p.getHeadSize()
      #  print "on_recv2*****************^^^"
        head=self._conn.recvall(head_size,timeout)
        DCE_APP().DEBUG("[SimpleProxy] recived head ")
        if len(head) !=head_size:
            DCE_APP().DEBUG("[SimpleProxy] Connection may be closed by peer")
            raise pyDCE.exception.CommunicateException("Connection may be closed by peer")
        (type,size,codec)=p.parseHead(head)
        print (type,size,codec)
        body=""
        while size>0:
            buf=self._conn.recvall(size,timeout)##raise CommunicateException
            body=body+buf
            size = size - len(buf)
        try:
